...
class train:
    def __init__(self, name='', num=0, arr_time='', dep_time='', src='', des='', day_of_travel='', seat_availability_in_1AC=0, seat_availability_in_2AC=0, seat_availability_in_SL=0, fare_1AC=0, fare_2AC=0, fare_SL=0):
        self.name=name
        self.num=num
        ...
        self.seats={'1AC':seat_availability_in_1AC, '2AC':seat_availability_in_2AC, 'SL':seat_availability_in_SL}
    ...
    def check_availability(self,coach='', ticket_num=0):
        coach=coach.upper()
        if coach not in ('1AC', '2AC', 'SL'):
            self.print_seat_availability()
            coach=input("Please enter the coach (1AC/2AC/SL) :")
        else:
...
class acceptors:
    '''class containing functions for accpeting and validating values properly'''
    def accept_uid():
        uid=0
        try:
            uid=int(input("Please enter the User ID :"))
        except ValueError:
            print("Please enter a proper User ID.")
    ...
    def accept_train_number():
        train_num=0
        try:
            train_num= int(input("Please enter the train number:"))
        except ValueError:
            print("Please enter the proper train number.")
    ...
    def accept_coach():
        coach= input("Please enter your coach :")
        if coach not in ('1AC', '2AC','SL'):
            print("Please enter the proper coach!!")
    ...
    def accept_ticket_num():
        ticket_num=0
        try:
            ticket_num= int(input("Please enter the number of tickets :"))
            if ticket_num <0:
                raise ValueError
        except ValueError:
            print("Please enter valid number of tickets :")
            return acceptors.accept_ticket_num()
        else:
            return ticket_num
    
    def accept_pnr():
        pnr=input("Please enter your PNR number :")
        if pnr not in ticket_dict:
            print("Please enter the valid PNR number :")
...
def book_ticket():
    ...
    if trains[choice].check_availability(coach, ticket_num):
        ...
        if prompt== 'Y':
            ...
            print("Kindly note PNR number :",tick.pnr, "\n\n")
            menu()
        else:
            print("Exiting...\n\n")
            menu()
...
def load():
	...
	with open("C:\\Users\91967\\data.pkl","rb") as f:
